...
class VideoPlayer:
    ...
    def video_source(self, path):
        ...
        self.cap = cv2.VideoCapture(self._video_source)
        if self.cap.isOpened():
            self._fps = self.cap.get(cv2.CAP_PROP_FPS)  # Get FPS from video file
...
def init_root_view():
    ...
    def set_parameters():
        ...
        try:
            ...
            assert Parameters.max_level_S > 0, "Max level S must be positive"
    ...
    def run_coding_thread():
        ...
        root.after(0, open_results_view)
    ...
    set_button = tk.Button(params_frame, text="Set Parameters", command=set_parameters)
...
def init_results_view():
    ...
    ttk.Checkbutton(checkboxes_frame, text='Show PSNR', variable=show_psnr_var, command=lambda: [show_ssim_var.set(False), show_ref_psnr_var.set(False),show_ref_ssim_var.set(False), update_quality_metrics_plot()]).pack(side=tk.LEFT)
    ttk.Checkbutton(checkboxes_frame, text='Show SSIM', variable=show_ssim_var, command=lambda: [show_psnr_var.set(False),show_ref_psnr_var.set(False),show_ref_ssim_var.set(False), update_quality_metrics_plot()]).pack(side=tk.LEFT)
    ttk.Checkbutton(checkboxes_frame, text='Show Refrerence PSNR', variable=show_ref_psnr_var, command=lambda: [show_ssim_var.set(False), show_psnr_var.set(False), show_ref_ssim_var.set(False), update_quality_metrics_plot()]).pack(side=tk.LEFT)
    ttk.Checkbutton(checkboxes_frame, text='Show Reference SSIM', variable=show_ref_ssim_var, command=lambda: [show_psnr_var.set(False),show_ssim_var.set(False), show_ref_psnr_var.set(False), update_quality_metrics_plot()]).pack(side=tk.LEFT)
...
def get_data():
    st_packets_data = pd.read_csv(os.path.join(Parameters.trace_file_path, 'st-packet'), sep = '\t', comment='#')
    st_packets_data.columns =  ["time", "seqNb", "pktSize", "frameNb", "frameType", "layerNb", "blocksList","signal_lost(db)","SNR","BER"] 
    
    st_frame_data = pd.read_csv(os.path.join(Parameters.trace_file_path, 'st-frame'), sep = '\t', comment='#')
    st_frame_data.columns = ["Rank", "Type", "Size(Bytes)", "refPSNR", "refSSIM", "bpp", "layers Size (bits)",
                           "captureEnergy(mJ)", "encodingEnergy(mJ)", "bit rate (kbps)"]
    rt_frame_data = pd.read_csv(os.path.join(Parameters.trace_file_path, 'rt-frame'), sep = '\t', comment='#')
    rt_frame_data.columns = ["Rank", "frameType", "PSNR", "SSIM"]
